...
async def _(event):
    ...
    if not event.reply_to_msg_id:
        await event.edit(
            "```King, Mohon Balas Ke Pesan Pengguna atau ketik .deteksi (ID/Username) Yang mau Anda deteksi```"
        )
        return
    if input_str:
        try:
            ...
            try:
                ...
                await edit.event(
                    "`King, Mohon Berikan ID/Username untuk menemukan Riwayat`"
                )
            uid = u.id
    else:
    ...
    async with bot.conversation(chat) as conv:
        try:
            ...
            await steal.reply("``King Mohon Unblock @tgscanrobot Dan Coba Lagi```")
