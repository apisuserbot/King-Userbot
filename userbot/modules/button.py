...
@register(outgoing=True, pattern="^.cbutton(?: |$)(.*)")
async def _(event):
...
@register(outgoing=True, pattern="^.ibutton(?: |$)(.*)")
async def _(event):
    ...
    if not markdown_note:
        return await edit_delete(event, "`what text should i use in button post`")
    input = "Inline buttons " + markdown_note
    results = await event.client.inline_query(BOT_USERNAME, input)
...
CMD_HELP.update(
    {
        ...
        \n\n  •  **Perintah :** `.cbutton`\
        \n  •  **Function : **Untuk Membuat Sebuah Button Dengan Bot\
    "
    }
)
